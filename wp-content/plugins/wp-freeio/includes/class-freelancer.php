...
class WP_Freeio_Freelancer {
	...
	public static function process_save_withdraw() {
        ...
        $verified = apply_filters( 'wp-freeio-calculate-freelancer-verified', $meta_obj->get_post_meta( 'verified' ), $user_id );
        if ( !$verified ) {
					...
					<ul class="payout-details-wrapper">
						...
						foreach ($payouts_details as $key => $value) {
							...
							switch ($key) {
								case 'payout_method':
									$all_payout_methods = WP_Freeio_Mixes::get_default_withdraw_payout_methods();
									$method_title = esc_html__('Payout Method', 'wp-freeio');
									$val = isset($all_payout_methods[$value]) ? $all_payout_methods[$value] : $value;
									break;
								case 'paypal_email':
									$method_title = esc_html__('Paypal Email', 'wp-freeio');
									$val = $value;
									break;
								case 'payoneer_email':
									$method_title = esc_html__('Payoneer Email', 'wp-freeio');
									$val = $value;
									break;
								default:
									$method_title = isset($bank_transfer_fields[$key]) ? $bank_transfer_fields[$key] : $key;
									$val = $value;
									break;
							}
							?>
							<li class="item">
								<span class="text"><?php echo esc_html($method_title); ?>:</span>
