...
class WP_Freeio_User {
	...
	public static function process_login() {
	    ...
	    if ( is_wp_error($user_signon) ){
			$result = array('status' => false, 'msg' => esc_html__('Wrong username or password. Please try again!!!', 'wp-freeio'));
	    } else {
	        ...
	        $result = array('status' => true, 'msg' => esc_html__('Signin successful, redirecting...', 'wp-freeio'), 'role' => $role);
	    }
	    $result = json_encode( apply_filters('wp-freeio-login-user-result-args', $result));
   		echo trim($result);
   		die();
	}
	...
	public static function process_register_new() {
		...
		if ( !get_option('users_can_register') ) {
			$return = array( 'status' => false, 'msg' => esc_html__('The website does not allow account registration.', 'wp-freeio') );
            wp_send_json( $return );
		}
	    ...
	    $jsondata = apply_filters('wp-freeio-register-user-result-args', $jsondata);
	...
	public static function process_change_password() {
		...
		if ( strlen( $new_password ) < 8 ) {
	        $reg_errors[] = esc_html__( 'Password length must be greater than 8 characters', 'wp-freeio' );
	    }

	    if ( ! preg_match( '/[0-9]/', $new_password ) ) {
            $reg_errors[] = esc_html__( 'Password must have at least 1 numeric character', 'wp-freeio' );
        }
        if ( ! preg_match( '/[a-z]/', $new_password ) ) {
            $reg_errors[] = esc_html__( 'Password must have at least 1 lower case character', 'wp-freeio' );
        }
        if ( ! preg_match( '/[A-Z]/', $new_password ) ) {
            $reg_errors[] = esc_html__( 'Password must have at least 1 upper case character', 'wp-freeio' );
        }

	    if ( $new_password != $retype_password ) {
	        $reg_errors[] = esc_html__( 'New and retyped password are not same.', 'wp-freeio' );
	    }
