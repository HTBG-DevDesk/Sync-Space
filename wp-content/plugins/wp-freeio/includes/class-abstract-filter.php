...
class WP_Freeio_Abstract_Filter {
	...
	public static function date_posted_options() {
		return apply_filters( 'wp-freeio-date-posted-options', array(
			array(
				'value' => '1hour',
				'text'	=> __('Last Hour', 'wp-freeio'),
			),
			array(
				'value' => '24hours',
				'text'	=> __('Last 24 hours', 'wp-freeio'),
			),
			array(
				'value' => '7days',
				'text'	=> __('Last 7 days', 'wp-freeio'),
			),
			array(
				'value' => '14days',
				'text'	=> __('Last 14 days', 'wp-freeio'),
			),
			array(
				'value' => '30days',
				'text'	=> __('Last 30 days', 'wp-freeio'),
			),
			array(
				'value' => 'all',
				'text'	=> __('All', 'wp-freeio'),
			),
		) );
	}

	public static function orderby($query_vars, $params, $prefix = WP_FREEIO_JOB_LISTING_PREFIX) {
		// Order
		if ( ! empty( $params['filter-orderby'] ) ) {
			switch ( $params['filter-orderby'] ) {
				...
				case 'published':
					...
					break;
				case 'price':
					$query_vars['meta_key'] = $prefix . 'price';
		...
		return apply_filters('wp-freeio-filter-orderby-query-vars', $query_vars, $params);
	}
	...
	public static function get_posts_by_distance($latitude, $longitude, $distance, $post_stype = 'job_listing') {
		...
		if ( apply_filters( 'wp_freeio_get_job_listings_cache_results', false ) ) {
			$to_hash         = json_encode( array($earth_distance, $latitude, $longitude, $latitude, $distance, $post_stype) );
			$query_args_hash = 'wp_freeio_' . md5( $to_hash . WP_FREEIO_PLUGIN_VERSION );
	...
	public static function filter_count($name, $term_id, $field) {
		...
		$params = array();
		if ( WP_Freeio_Abstract_Filter::has_filter() ) {
			$params = $_GET;
		}
		...
		$cached_counts = (array) get_transient( 'wp_freeio_filter_counts' );
		if ( ! isset( $cached_counts[ $query_hash ] ) ) {
			$loop = WP_Freeio_Query::get_posts($args, $params);
			$cached_counts[ $query_hash ] = $loop->found_posts;
			set_transient( 'wp_freeio_filter_counts', $cached_counts, DAY_IN_SECONDS );
		}
	...
	public static function filter_get_name($key, $field) {
		...
		$name = $prefix.'-'.$key;
		return apply_filters('wp-freeio-filter-get-name', $name, $key, $field);
	}
	...
	public static function filter_field_input($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/text' );
	}
	...
	public static function filter_field_input_location($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/text_location' );
	}

	public static function filter_field_input_distance($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : apply_filters( 'wp_freeio_filter_distance_default', 50 );

		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/distance' );
	}

	public static function filter_field_input_date_posted($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : 'all';
		$options = WP_Freeio_Abstract_Filter::date_posted_options();
		
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/radios' );
	}

	public static function filter_field_found_date_range_slider($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';
		$min_max = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_EMPLOYER_PREFIX.'founded_date', 'employer');
		if ( empty($min_max) ) {
			return;
		}
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/range_slider' );
	}
	...
	public static function filter_field_job_salary($instance, $args, $key, $field) {
		...
		if ( !empty($_GET['filter-salary_type']) ) {
			$condition = array(
				'key' => WP_FREEIO_JOB_LISTING_PREFIX.'salary_type',
				'value' => $_GET['filter-salary_type']
			);
		}

		$salary_min = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_JOB_LISTING_PREFIX.'salary', 'job_listing', $condition);
		$salary_max = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_JOB_LISTING_PREFIX.'max_salary', 'job_listing', $condition);
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/salary_range_slider' );
	}
	
	public static function filter_field_freelancer_salary($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';

		$condition = array();

		$price_min = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_FREELANCER_PREFIX.'min_rate', 'freelancer', $condition);
		$price_max = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_FREELANCER_PREFIX.'max_rate', 'freelancer', $condition);
	...
	public static function filter_field_service_price($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';

		$condition = array();

		$min_max = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_SERVICE_PREFIX.'price', 'service', $condition);
	...
	public static function filter_field_project_price($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';

		$condition = array();

		$price_min = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_PROJECT_PREFIX.'price', 'project', $condition);
		$price_max = WP_Freeio_Query::get_min_max_meta_value(WP_FREEIO_PROJECT_PREFIX.'max_price', 'project', $condition);
		if ( empty($price_min) && empty($price_max) ) {
			return;
		}
		$min = $max = 0;
	...
	public static function filter_field_checkbox($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/checkbox' );
	}
	...
	public static function filter_field_select($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/select' );
	}

	public static function filter_field_multiselect($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/multiselect' );
	}
	...
	public static function filter_field_check_list($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/check_list' );
	}
	...
	public static function filter_field_taxonomy_radio_list($instance, $args, $key, $field) {
		...
		$options = array();
		$query_args = apply_filters('wp-freeio-filter-field-taxonomy-radio-list-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
	...
	public static function filter_field_taxonomy_check_list($instance, $args, $key, $field) {
		...
		$options = array();
		$query_args = apply_filters('wp-freeio-filter-field-taxonomy-check-list-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
		$terms = get_terms($field['taxonomy'], $query_args);
	...
	public static function filter_field_taxonomy_select($instance, $args, $key, $field) {
		...
		$options = array();
		$query_args = apply_filters('wp-freeio-filter-field-taxonomy-taxonomy-select-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
		$terms = get_terms($field['taxonomy'], $query_args);
	...
	public static function filter_field_taxonomy_hierarchical_radio_list($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/tax_radios' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_check_list($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/tax_check_list' );
	}

	public static function filter_field_taxonomy_hierarchical_multiselect($instance, $args, $key, $field) {
		$name = self::filter_get_name($key, $field);
		$selected = !empty( $_GET[$name] ) ? $_GET[$name] : '';

		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/tax_multiselect' );
	}
	...
	public static function filter_field_taxonomy_hierarchical_select($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/tax_select' );
	}
	...
	public static function filter_field_taxonomy_select_search($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/tax_select_search' );
	}
	...
	public static function filter_field_location_select($instance, $args, $key, $field) {
	    ...
	    if ( $location_type === 'no' ) {
	    	...
	    	include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/regions_select' );
	    }
	}
	...
	public static function filter_field_location_select_search($instance, $args, $key, $field) {
	    ...
	    if ( $location_type === 'no' ) {
	    	...
	    	include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/regions_select_search' );
	    }
	}
	...
	public static function hierarchical_tax_tree($catId, $depth, $input_name, $key, $field, $selected, $input_type = 'checkbox'){
	    ...
	    $query_args = apply_filters('wp-freeio-filter-field-taxonomy-tax-tree-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
	...
	public static function hierarchical_tax_option_tree($catId, $depth, $input_name, $key, $field, $selected ){
	    ...
	    $query_args = apply_filters('wp-freeio-filter-field-taxonomy-option-tree-query-args', array( 'hierarchical' => 1, 'hide_empty' => true, 'orderby' => 'title'  ), $field);
	    $query_args['parent'] = $catId;
		$terms = get_terms($field['taxonomy'], $query_args);
	...
	public static function filter_field_employers($instance, $args, $key, $field) {
		...
		$options = array();
		$employer_ids = WP_Freeio_User::get_author_employers();
	...
	public static function filter_field_rating_select($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/rating_select' );
	}
	...
	public static function filter_field_rating_multiselect($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/rating_multiselect' );
	}
	...
	public static function filter_field_rating_checkbox($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/rating_checkbox' );
	}
	...
	public static function filter_field_rating_radio($instance, $args, $key, $field) {
		...
		include WP_Freeio_Template_Loader::locate( 'widgets/filter-fields/rating_radio' );
	}
	...
	public static function get_the_level($id, $type = 'location') {
